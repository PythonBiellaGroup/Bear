...
import numpy as np
import pandas as pd
from sniply.src.conversions import date2millis, millis2date
...
def str_to_date(x: str, date_format: str = "%Y-%m-%d") -> dt.date:
    ...
    """
    return dt.datetime.strptime(x, date_format).date()  # noqa: DTZ007
...
def str_to_datetime(x: str, date_format: str = "%Y-%m-%d %H:%M:%S") -> dt.datetime:
    ...
    """
    return dt.datetime.strptime(x, date_format)  # noqa: DTZ007
...
def acquire_date(x: Optional[Any], date_format: str = "%Y-%m-%d %H:%M:%S") -> Any:
    ...
    """
    if isinstance(x, str):
        return dt.datetime.strptime(x, date_format).date() if not pd.isnull(x) else np.nan  # noqa: DTZ007
